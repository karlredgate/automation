...
struct parse_cmd {
 ...
 class qtreefactory {
  public:
     ...
     //
     // expected, if supplied, usually means that this is what
     // we are expecting and we just want to ignore that
     // token... like ')', or ']' or '}'
     ...
     //    it binds to tokens on its right. expression calls the nud
     //    method of the token.
     //    The nud is used to process literals, variables, and prefix
     //    operators. Then as long as the right binding power is less
     //    than the left binding power of the next token, the led method
     //    is invoked on the following token. The led is used to process
     //    infix and suffix operators.
     //    This process can be recursive because the nud and led methods
     //    can call expression.
     //
  ...
  private:
     ...
     // explicity the cnode expression stack, which would normally
     // hold t / cnode.
     // Recall that 't' in Pratt's paper became cnode (current node)
     // in my renaming for better documentation effort.
 ...
 // Very Important NO VTABLE note: subclasses cannot add their own members,
 // since they will all be deleted via a qtree*.
 // Any additional data members must go into qtree itself.
 // This is so we can get rid of the stupid vtable, and thus track
 // the memory accurately using the Tag/_ser number system.
 //
