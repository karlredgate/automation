...
//     "The parse Tree Up to this point, by consuming the tokens to the left"
//     would be a better-but-too-long name.
//
...
//                                  _global_next_token (or _token)
//                                  contains the following token.
//
//                  actually we just keep '_token' as in, rather than this
//                  more verbose description we considered:
//                _token -> _global_next_token_node
//                (since it is a global variable and can/will be incremented by many routines)
//
//  meaning of rbp parameter: if you hit a token with a  _token->_lbp < rbp,
//  then don't bother calling munch_left, stop and return what you have.
//
// better explanation:  rbp = a lower bound on descendant nodes precedence level,
// so we can guarantee the precenence-hill-climbing property (small precedence
...
// isolate just the "and parse" of "lex_and_parse",
// so that CopyFrom can use it without repeating the lexing.
// return the number of characters consumed,
// so we know where to start parsing next.
// add parsed sexp_t* to client_parse_cmd._rstk as well
// as our intnernal _parsed_ustaq
long tdopout_struct::and_parse(parse_cmd*  client_parse_cmd) {
   ...
   // 4) and parse... re-parse everything from the start.
    ...
    while(!_qf.eof()) {
        ...
        //    the argument is the lower bound on precedence below us, and needs
	//    to be zero or greater, 0 here to stop after parsing one tree.
...
//
//  ownership: the tdop can be killed at the end of this call.
//  Clients should transfer the sealed sexp to the client's ptag,
//  before doing so. pointers to the new sexp are already on the
//  client's stk, so this should be easy.
//
//  tdop are no longer marked as sysbuiltin so that we can track
//  them if they leak.
