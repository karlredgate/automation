tell application "iTerm 2"
	...
	tell SI1terminal
		...
		tell SI1a
			...
			«event ITRMExec» given «class Cmnd»:"ssh -2 lvm-ap-northeast-1.scoe-sil.net"
		end tell
		
		set SI1b to (make new «class Pssn» at the end of every «class Pssn»)
		tell SI1b
			set name to "Asia Pacific (Singapore)"
			«event ITRMExec» given «class Cmnd»:"ssh -2 lvm-ap-southeast-1.scoe-sil.net"
		end tell
		
		set SI1c to (make new «class Pssn» at the end of every «class Pssn»)
		tell SI1c
			set name to "Asia Pacific (Sydney)"
			«event ITRMExec» given «class Cmnd»:"ssh -2 lvm-ap-southeast-2.scoe-sil.net"
		end tell
		
		set SI1d to (make new «class Pssn» at the end of every «class Pssn»)
		tell SI1d
			set name to "EU (Frankfurt)"
			«event ITRMExec» given «class Cmnd»:"ssh -2 lvm-eu-central-1.scoe-sil.net"
		end tell
		
		set SI1e to (make new «class Pssn» at the end of every «class Pssn»)
		tell SI1e
			set name to "EU (Ireland)"
			«event ITRMExec» given «class Cmnd»:"ssh -2 lvm-eu-west-1.scoe-sil.net"
		end tell
		
		set SI1e to (make new «class Pssn» at the end of every «class Pssn»)
		tell SI1e
			set name to "US East (N. Virginia)"
			«event ITRMExec» given «class Cmnd»:"ssh -2 lvm-us-east-1.scoe-sil.net"
		end tell
		
		set SI1e to (make new «class Pssn» at the end of every «class Pssn»)
		tell SI1e
			set name to "US West (Oregon)"
			«event ITRMExec» given «class Cmnd»:"ssh -2 lvm-us-west-2.scoe-sil.net"
		end tell
